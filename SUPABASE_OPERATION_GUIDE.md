...
### A. 管理者がユーザーを招待する (メールリンクによる登録)
管理者がSupabaseの管理画面からスタッフのメールアドレスを入力し、招待メールを送信する方法です。これがご質問の「招待とリアクションによる登録」に該当します。

- **手順**: 
  1. 管理者が Supabase Dashboard > Authentication > Users > 「Invite user」をクリック。
  2. スタッフのメールアドレスを入力して送信。
  3. **スタッフ（ユーザー）のリアクション**: 届いたメール内の「Accept Invitation」リンクをクリック。
  4. アプリケーションにリダイレクトされ、自動的に「確認済みユーザー」として登録・ログインされます。
- **メリット**: パスワードの設定をユーザー自身に任せることができ、管理者側で初期パスワードを管理する手間が不要です。
- **初期設定モード**: 招待後のリダイレクト先でパスワードを設定する画面を設けるのが一般的ですが、パスワードなし（マジックリンク）での運用も可能です。
...
## 付録：具体的なアカウント追加手順 (方法1: 管理者が作成)

管理者がパスワードを決めてスタッフに教える場合の、具体的な操作手順です。

1.  **Supabase Dashboard** にログインし、対象のプロジェクトを選択します。
2.  左端のメニューから **「Authentication」** (鍵のアイコン) をクリックします。
3.  **「Users」** タブが開かれていることを確認し、右上の **「Add User」** ボタンをクリックして **「Create new user」** を選択します。
4.  スタッフの **「Email」** と、あなたが決めた初期 **「Password」** を入力します。
5.  **「Auto-confirm user?」** のスイッチを **ON** にします。（※これが重要です。ONにすると、スタッフがメールの中身を確認しなくても、すぐにログインできるようになります）
6.  **「Send invitation email?」** は **OFF** にしても構いません（あなたが直接パスワードを伝える場合）。
7.  **「Create user」** をクリックして完了です。

登録したメールアドレスとパスワードをスタッフに伝えれば、すぐにアプリを使えるようになります。
